...
async function main(): Promise<void> {
    ...
    const connection: web3.Connection = new web3.Connection(endpoint, "finalized");
    ...
    const tokenMint = await token.createMint(connection, local_account, local_account_publickey, local_account_publickey, 2);

    const link = helpers.getExplorerLink("address", tokenMint.toString(), "localnet");
